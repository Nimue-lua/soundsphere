...
---@param env yi.Environment
function View:mount(env)
	self.env = env
	self:load()
...
function View:add(view, params)
	...
	---@cast view yi.View
	if view.state ~= State.AwaitsMount then
		view:mount(self.env)
	end
...
function View:isMouseOver(mouse_x, mouse_y)
	local imx, imy = self.transform:inverseTransformPoint(mouse_x, mouse_y)
...
function View:updateTransforms()
	local x_axis = self.layout_box.x
	local y_axis = self.layout_box.y
	local tf_params = self.transform_params
	local ox = tf_params.origin_x * self:getCalculatedWidth()
	local oy = tf_params.origin_y * self:getCalculatedHeight()

	if self.parent then
		local ax = x_axis.pos + tf_params.anchor_x * self.parent:getCalculatedWidth()
		local ay = y_axis.pos + tf_params.anchor_y * self.parent:getCalculatedHeight()
		-- Using temp_tf to avoid allocating a new transform object
		...
		self.transform:setTransformation(
			x_axis.pos + tf_params.x,
			y_axis.pos + tf_params.y,
			tf_params.angle,
			tf_params.scale_x,
			tf_params.scale_y,
			ox,
